...
func (r *TransactionMySQL) Get(transaction *[]entity.Transaction, param dto.TransactionParam, pagination dto.PaginationRequest) error {
	return r.db.Debug().Preload(clause.Associations).Preload("TransactionDetails.Product").Limit(pagination.Limit).Offset(pagination.Offset).Where("status != ?", string(entity.CancelledBySystem)).Order("created_at desc").Find(transaction, param).Error
}
